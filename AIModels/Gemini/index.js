...
export const geminiChat = async (req, res, ai) => {
    try {
        ...
        // console.log("sdklfahvlasd ------------------- ", req.body, "exte ----------------------- ", isExternal);
        ...
        sseManager.send(clientId, "message", { role: "user", text: message, conversationId });
        ...
        // console.log("toGeminiMessages(sess.messages)-------------------------------->:", toGeminiMessages(sess.messages));
        ...
        if (fnCall?.name) {
            ...
            if (name === "insights_ae" || name === "storefront_database_ae") {
               console.log("hit");
                ...
                console.log("-----------------Summary resultText:", summaryResp.candidates?.[0]?.content);
            ...
            if (isExternal) {
                return res.json({ role: "model", text: resultText, conversationId, toolName: name });
            } else {
                return res.json({ ok: true, conversationId }); // <-- no res for /chat
            }
        }
        ...
        sseManager.send(clientId, "message", { role: "model", text: finalText, conversationId });

        if (isExternal) {
            return res.json({ role: "model", text: finalText, conversationId });
        } else {
            return res.json({ ok: true, conversationId });; // <-- no res for /chat
        }
    } catch (err) {
